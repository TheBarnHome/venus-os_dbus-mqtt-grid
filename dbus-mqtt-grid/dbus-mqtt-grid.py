...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        ...
        connected = 1
        logging.warning(config["MQTT"]["TOPICS"])
    ...
    try:
        ...
        if msg.topic in config["TOPICS"]:
            if msg.payload != "" and msg.payload != b"":
                ...
                if msg.topic == config["TOPICS"]["topic_total_power"]:
                    grid_power = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_total_current"]:
                    grid_current = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_total_voltage"]:
                    grid_voltage = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_total_energy_in"]:
                    grid_forward = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_total_energy_out"]:
                    grid_reverse = float(msg.payload)

                if msg.topic == config["TOPICS"]["topic_L1_power"]:
                    grid_L1_power = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L1_current"]:
                    grid_L1_current = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L1_voltage"]:
                    grid_L1_voltage = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L1_frequency"]:
                    grid_L1_frequency = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L1_energy_in"]:
                    grid_L1_forward = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L1_energy_out"]:
                    grid_L1_reverse = float(msg.payload)
                
                if msg.topic == config["TOPICS"]["topic_L2_power"]:
                    grid_L2_power = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L2_current"]:
                    grid_L2_current = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L2_voltage"]:
                    grid_L2_voltage = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L2_frequency"]:
                    grid_L2_frequency = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L2_energy_in"]:
                    grid_L2_forward = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L2_energy_out"]:
                    grid_L2_reverse = float(msg.payload)

                if msg.topic == config["TOPICS"]["topic_L3_power"]:
                    grid_L3_power = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L3_current"]:
                    grid_L3_current = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L3_voltage"]:
                    grid_L3_voltage = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L3_frequency"]:
                    grid_L3_frequency = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L3_energy_in"]:
                    grid_L3_forward = float(msg.payload)
                if msg.topic == config["TOPICS"]["topic_L3_energy_out"]:
                    grid_L3_reverse = float(msg.payload)
            else:
                logging.warning(
